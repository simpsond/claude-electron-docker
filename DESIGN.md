...
**Discovered constraint: `--remote-debugging-port` is gated; the Node inspector route survives.** This entry has been revised twice and is worth reading in full. An earlier version of this design listed an absent `--remote-debugging-port` as an accepted risk, with the assumption that adding it later would be a one-line change. Implementation testing falsified that assumption: the Claude Desktop main process executes `DF(process.argv) && !oP() && process.exit(1)` early in startup, where `DF` matches `--remote-debugging-port` or `--remote-debugging-pipe` on argv (with any leading `--`/`-`/`/` stripped, case-insensitive), and `oP` validates an Ed25519-signed `CLAUDE_CDP_AUTH` token (three-segment dotted, JWT-shaped, with a five-minute freshness window) against a public key embedded in the app bundle. The signing key is held by Anthropic; we cannot forge a token. So the gate exists, it is intentional, and it cannot be opened from outside.

A second revision of this entry concluded that the gate ruled out CDP-style automation entirely, and that the eventual test harness would have to drive Claude through xdotool, AT-SPI, and OCR. That was an overcorrection. The aaddrick project's own test harness routes around the gate using the **Node inspector** (port 9229), a different surface that `DF` does not inspect. The path is documented in the upstream repo at [`docs/testing/automation.md`](https://github.com/aaddrick/claude-desktop-debian/blob/main/docs/testing/automation.md), and the in-app `Developer → Enable Main Process Debugger` menu item opens the same code path — i.e., this route is tolerated by upstream, not an exploit. The mechanics:

1. Spawn Electron with no debug-port flags. The gate stays asleep.
2. Wait for the X11 window to appear (signal that the app is up).
3. Send `SIGUSR1` to the main-process PID. `inspector.open()` runs at runtime and the Node inspector listens on port 9229.
4. Connect over WebSocket and use `Runtime.evaluate` to run JS in the main process. From there, `webContents.getAllWebContents()` enumerates live web contents (the shell window plus the embedded `claude.ai` BrowserView), and `webContents.executeJavaScript(...)` drives renderer-side state.

Two implementation gotchas the upstream runbook records, captured here so we do not rediscover them:

- `BrowserWindow.getAllWindows()` returns 0 in this build because `frame-fix-wrapper` substitutes the `BrowserWindow` class and breaks the static registry. Use `webContents.getAllWebContents()` instead — that registry stays intact.
- `Runtime.evaluate` with `awaitPromise: true` + `returnByValue: true` returns empty objects for awaited Promise resolutions on this V8 build. Wrap return values in `JSON.stringify` on the evaluated side and `JSON.parse` on the caller.

Implication for this PoC: nothing changes today. Section 0's success criterion is interactive use over VNC, which is met. `start-claude.sh` correctly omits `--remote-debugging-port` (passing it crashes the app). The CDP work is firmly a follow-on for the test-harness migration, and when that work begins this section is the entry point. The harness needs (a) the main-process PID, available from `pgrep -f 'electron.*app.asar' | head -1` inside the container, and (b) port 9229 reachable from the host, which is one additional `127.0.0.1:9229:9229` line in the compose port map plus a documented `docker exec claude-desktop kill -SIGUSR1 <pid>` invocation. Both are deferred until the harness work actually starts.
